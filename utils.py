...
def download_returns(ticker, period="max"):
    if isinstance(period, _pd.DatetimeIndex):
        ...
        p = {"period": period}
    # SVK start
    from nsepy import get_history as _gh # SVK
    from datetime import date # SVK
    import nsedata.history as history # SVK2
    PATH = 'D:/Trading/Historical Data/' # SVK2 Root directory path for following directories
    DBPATH = 'db/nsedb.db'               # SVK2 Relative directory path for sqlite database to store data
    hist =  history.History(path=PATH, db=DBPATH) # SVK2
    returns = hist.symbol_history(symbol=ticker,
                          start_date='19950101',
                          end_date='20200820',
                         )['Close'].pct_change() # SVK2
    #returns = _gh(symbol=ticker, start=date(1995,1,1), end=date.today(), index=True)['Close'].pct_change() # SVK
    returns.index = _pd.to_datetime(returns.index) # SVK
    if returns.empty is True: # SVK Symbol not listed in NSE
        returns = _yf.Ticker(ticker).history(**p)['Close'].pct_change() # SVK
    return returns # SVK
    #return _yf.Ticker(ticker).history(**p)['Close'].pct_change() # SVK
    # SVK end
