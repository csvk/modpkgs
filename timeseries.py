...
def rolling_fama_french(returns, factor_returns=None,
    ...
    for beg, end in zip(factor_returns.index[:-rolling_window],
        ...
        pkl.dump([factor_returns[beg:end], returns[beg:end]], open('dump.obj', "wb" ))
        # SVK end
...
def get_max_drawdown_underwater(underwater):
    ...
    SVK end'''
    # SVK start
    idx_valley = np.argmin(underwater)  # end of the period
    valley = underwater.index[idx_valley]
    # Find first 0
    peak = underwater[:idx_valley][underwater[:idx_valley] == 0].index[-1]
    # Find last 0
    try:
        recovery = underwater[idx_valley:][underwater[idx_valley:] == 0].index[0]
    # SVK end
    except IndexError:
