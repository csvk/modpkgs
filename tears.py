...
def create_full_tear_sheet(returns,
    ...
    if benchmark_rets is None:
        # SVK start
        # benchmark_rets = utils.get_symbol_rets('SPY') #SVK
        benchmark_rets = returns.copy()
        benchmark_rets.values[:] = 0
        #SVK end
...
def create_returns_tear_sheet(returns, positions=None,
    ...
    #plotting.plot_rolling_fama_french( SVK
    #    returns, ax=ax_rolling_risk) SVK

    # Drawdowns
    plotting.plot_drawdown_periods(
        returns, top=3, ax=ax_drawdown) # SVK top=3 changed from top=5
